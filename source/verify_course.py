...
numeric={2:2*4+1,4:5*6,7:[[[2,8],[5,1]],[[9,4],[6,3]]][0][1][0],8:sum(i*i for i in range(1,4)),9:1*3+4*2,
 13:math.dist([2,-1],[7,11]),16:8*.25,17:((1-4)**2+(7-4)**2)/2,18:3,19:2*4,
 21:2*(3*2)*3,22:4*5,23:min(range(4),key=lambda x:(x-5)**2),24:4-.1*6,
 26:(27-15)/4,30:4*3+2,31:(1**2+3**2)/2,33:2*3-1*4+1,34:1/(1+3),35:max(0,-2)+max(0,3),
 39:(-2+6)/2,40:2*(120/30),44:.75,47:0,52:2,57:.2*(-2)+.3*4+.5*8,61:.4*.5,
 65:8+4-3,68:1+.5*4,70:(4+8)/2,71:3+.5*4,72:-2+.9*10,74:.7*6+.3*(-2),
 79:2**4,80:abs(2.718-2.72),83:2*3+4,84:8*128,86:18/300,96:2500*1.2/100}
